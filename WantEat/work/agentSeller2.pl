...
a(checkRequired(var_N,var_Q,var_T,var_M)):-quantita(var_Q_ag),tempo(var_T_ag),var_Q_ag=<var_Q,var_T_ag=<var_T,ordine(var_O1),random(1,10,var_Ran),var_G is 20-var_Ran,var_G=<var_M,a(message(agentManager,send_message(offerFromSeller(var_N,var_Q,var_T,var_M,var_G,0,agentSeller2),agentSeller2))),write(' OFFERTA INVIATA PER SERVIRE ORDINE N: '),write(var_O1),nl,nl.

a(checkRequired(var_N,var_Q,var_T,var_M)):-ordine(var_O),write(' REQUISITI INSUFFICIETNI SE SODDISFARE L ORDINE N: '),write(var_O),nl,a(message(agentManager,send_message(refuseFromSeller(var_N,var_Q,var_T,var_M,0,agentSeller2),agentSeller2))),nl,nl.

eve(offerRejected(var_N)):-ordine(var_O),write('--> RIFIUTATA L OFFERTA PER L ORDINE: '),write(var_O),nl,nl.

eve(offerAccepted(var_N,var_Q,var_T,var_M,var_G)):-ordine(var_O),write('--> ACCETTATA L OFFERTA PER L ORDINE: '),write(var_O),nl,write(' PRODOTTO CONSEGNATO: '),write(var_N),nl,write(' GUADAGNO OTTENUTO: '),write(var_G),nl,nl,guadagno(var_B),retractall(guadagno(var__)),var_B1 is var_B+var_G,assert(guadagno(var_B1)),a(message(agentManager,send_message(orderAssigned(var_N,agentSeller2),agentSeller2))),write(' ORDINE N: '),write(var_O),write(' SODDISFATTO CON SUCCESSO!! '),nl,nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_372003,var_Ontology,_372007),_371997),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_372041)),a(message(var_Ag,propose(var_A,[_372041],var_AgI))),retractall(ext_agent(var_Ag,_372079,var_Ontology,_372083)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_371877,var_Ontology,_371881),_371871),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_371947,var_Ontology,_371951)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_371765,var_Ontology,_371769),_371759),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_371821,var_Ontology,_371825)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_371329),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_371363),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_371145).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_370993),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_370941),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_370817,_370819)),clause(agente(_370839,_370841,_370843,var_S),_370835),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_370611,_370613,_370615,var_S),_370607),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_370711,[]),repeat,read(_370711,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_370711).
