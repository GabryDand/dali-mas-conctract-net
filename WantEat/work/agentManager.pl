...
eve(newOrder(var_N,var_Q,var_T,var_M)):-num_ordine(var_O),write(' ORDINE RICHIESTO: '),write(var_N),nl,write(' OFFERTA PER SERVIRE L ORDINE N: '),write(var_O),nl,retractall(order(var__,var__,var__,var__,var__)),assert(order(var_N,var_Q,var_T,var_M,0)),retractall(bestOffer(var__,var__,var__,var__,var__,var__,var__)),a(message(agentSeller1,send_message(newOrder(var_N,var_Q,var_T,var_M),agentManager))),a(message(agentSeller2,send_message(newOrder(var_N,var_Q,var_T,var_M),agentManager))),a(message(agentSeller3,send_message(newOrder(var_N,var_Q,var_T,var_M),agentManager))),a(message(agentSeller4,send_message(newOrder(var_N,var_Q,var_T,var_M),agentManager))).

eve(closeApplication(var_E)):-nl,write(' SESSIONE TERMINATA NON ESSENDOCI PIU ORDINI DA SODDISFARE '),nl,a(message(agentSeller1,send_message(closeApplication(var_E),agentManager))),a(message(agentSeller2,send_message(closeApplication(var_E),agentManager))),a(message(agentSeller3,send_message(closeApplication(var_E),agentManager))),a(message(agentSeller4,send_message(closeApplication(var_E),agentManager))).

eve(offerFromSeller(var_N,var_Q,var_T,var_M,var_G,0,var_A)):-bestOffer(var__,var__,var__,var__,var__,var__,var_A_p),bestPrice(var_G_min),var_G<var_G_min,num_ordine(var_O),a(message(var_A_p,send_message(offerRejected(var_N),agentManager))),write(' ACCETTATA DA: '),write(var_A),nl,write(' AL PREZZO DI: '),write(var_G),nl,retractall(bestPrice(var__)),assert(bestPrice(var_G)),retractall(bestOffer(var__,var__,var__,var__,var__,0,var__)),assert(bestOffer(var_N,var_Q,var_T,var_M,var_G,0,var_A)),offerArrived(var_X),retractall(offerArrived(var__)),var_Z is var_X+1,assert(offerArrived(var_Z)).

eve(offerFromSeller(var_N,var_Q,var_T,var_M,var_G,0,var_A)):-bestPrice(var_G_min),var_G<var_G_min,write(' ACCETTATA DA: '),write(var_A),nl,write(' AL PREZZO DI: '),write(var_G),nl,nl,retractall(bestPrice(var__)),assert(bestPrice(var_G)),retractall(bestOffer(var__,var__,var__,var__,var__,0,var__)),assert(bestOffer(var_N,var_Q,var_T,var_M,var_G,0,var_A)),offerArrived(var_X),retractall(offerArrived(var__)),var_Z is var_X+1,assert(offerArrived(var_Z)).

eve(offerFromSeller(var_N,var_Q,var_T,var_M,var_G,0,var_A)):-bestPrice(var_G_min),var_G>=var_G_min,write(' ACCETTATA DA: '),write(var_A),nl,write(' AL PREZZO DI: '),write(var_G),nl,a(message(var_A,send_message(offerRejected(var_N),agentManager))),nl,offerArrived(var_X),retractall(offerArrived(var__)),var_Z is var_X+1,assert(offerArrived(var_Z)).

eve(refuseFromSeller(var_N,var_Q,var_T,var_M,0,var_A)):-write(' RIFIUTATA DA: '),write(var_A),nl,offerArrived(var_X),retractall(offerArrived(var__)),var_Z is var_X+1,assert(offerArrived(var_Z)).

assignOrderToSeller:-offerArrived(4),bestOffer(var_N,var_Q,var_T,var_M,var_G,0,var_A).

evi(assignOrderToSeller):-retractall(offerArrived(var__)),assert(offerArrived(0)),num_ordine(var_O),bestOffer(var_N,var_Q,var_T,var_M,var_G,0,var_A),a(message(var_A,send_message(offerAccepted(var_N,var_Q,var_T,var_M,var_G),agentManager))),sleep(1),write(' PER L ORDINE N: '),write(var_O),write(' ACCETTATA L OFFERTA DI '),write(var_A),nl,nl.

orderNotSatisfable:-offerArrived(4),bestPrice(9999).

evi(orderNotSatisfable):-retractall(offerArrived(var__)),assert(offerArrived(0)),num_ordine(var_O),write(' NESSUN OFFERTA PERVENUTA '),nl,write(' IMPOSSIBILE SODDISFARE L ORDINE N: '),write(var_O),num_ordine(var_O),write(var_O),nl,nl,var_O1 is var_O+1,retractall(num_ordine(var__)),assert(num_ordine(var_O1)),order(var_N,var__,var__,var__,0),a(message(organizer,send_message(orderNotFound(var_N),agentManager))),retractall(order(var__,var__,var__,var__,0)).

eve(orderAssigned(var_N,var_A)):-bestPrice(var_T),retractall(bestPrice(var__)),assert(bestPrice(9999)),num_ordine(var_O),write(' ORDINE N: '),write(var_O),num_ordine(var_X),write(' SODDISFATTO CON SUCCESSO'),nl,nl,num_ordine(var_O),var_O1 is var_O+1,retractall(num_ordine(var__)),assert(num_ordine(var_O1)),a(message(organizer,send_message(orderDelivered(var_N,var_A),agentManager))),retractall(order(var__,var__,var__,var__,0)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_432501,var_Ontology,_432505),_432495),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_432539)),a(message(var_Ag,propose(var_A,[_432539],var_AgI))),retractall(ext_agent(var_Ag,_432577,var_Ontology,_432581)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_432375,var_Ontology,_432379),_432369),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_432445,var_Ontology,_432449)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_432263,var_Ontology,_432267),_432257),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_432319,var_Ontology,_432323)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_431827),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_431861),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_431643).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_431491),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_431439),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_431315,_431317)),clause(agente(_431337,_431339,_431341,var_S),_431333),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_431109,_431111,_431113,var_S),_431105),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_431209,[]),repeat,read(_431209,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_431209).
