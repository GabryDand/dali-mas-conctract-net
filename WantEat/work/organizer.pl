...
evi(requestOrder):-order(var_N,var_Q,var_T,var_M,0),a(message(agentManager,send_message(newOrder(var_N,var_Q,var_T,var_M),organizer))),numOrder(var_Num),var_N1 is var_Num+1,retractall(numOrder(var__)),assert(numOrder(var_N1)),write('ORDINE UTENTE: '),write(var_N),nl,write('RICHIESTA INVIATA CON SUCCESSO AL MANAGER'),nl,nl,retractall(stop(var__)),assert(stop(1)).

closeApplication:-totOrder(6),on(1).

evi(closeApplication):-nl,write('*****APPLICAZIONE TERMINATA CON SUCCESSO*****'),nl,write(' Numero di ordini soddisfatti: '),numDelivered(var_Num1),numNotDelivered(var_Num2),write(var_Num1/6),a(message(agentManager,send_message(closeApplication('end'),organizer))),retractall(on(var__)),assert(on(0)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_371525,var_Ontology,_371529),_371519),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_371563)),a(message(var_Ag,propose(var_A,[_371563],var_AgI))),retractall(ext_agent(var_Ag,_371601,var_Ontology,_371605)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_371399,var_Ontology,_371403),_371393),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_371469,var_Ontology,_371473)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_371287,var_Ontology,_371291),_371281),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_371343,var_Ontology,_371347)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_370851),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_370885),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_370667).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_370515),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_370463),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_370339,_370341)),clause(agente(_370361,_370363,_370365,var_S),_370357),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_370133,_370135,_370137,var_S),_370129),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_370233,[]),repeat,read(_370233,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_370233).
