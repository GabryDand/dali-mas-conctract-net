...
order(contorni,2,50,20,0).

order(dolce,1,80,15,0).

order(bevanda,3,20,15,0).

eve(activateApplication):-retract(on(0)),assert(on(1)),write('*****APPLICAZIONE AVVIATA CON SUCCESSO*****'),nl,nl,nl,sleep(2).

requestOrder:-on(1),stop(0),order(var__,var__,var__,var__,0).

evi(requestOrder):-order(var_N,var_Q,var_P,var_M,0),a(message(agentManager,send_message(newOrder(var_N,var_Q,var_P,var_M),pippo))),numOrder(var_Num),var_N1 is var_Num+1,retractall(numOrder(var__)),assert(numOrder(var_N1)),write('ORDINE UTENTE: '),write(var_N),nl,write('RICHIESTA INVIATA CON SUCCESSO AL MANAGER'),nl,nl,retractall(stop(var__)),assert(stop(1)).

closeApplication:-totOrder(6),on(1).

evi(closeApplication):-nl,write('*****APPLICAZIONE TERMINATA CON SUCCESSO*****'),nl,write(' Numero di ordini soddisfatti: '),numDelivered(var_Num1),numNotDelivered(var_Num2),write(var_Num1/6),a(message(agentManager,send_message(closeApplication('end'),pippo))),retractall(on(var__)),assert(on(0)).

eve(orderDelivered(var_N,var_A)):-on(1),stop(1),retractall(order(var_N,var__,var__,var__,var__)),assert(order(var_N,0,0,0,4)),retractall(stop(var__)),assert(stop(0)),write('ORDINE: '),write(var_N),write(' SODDISFATTO DA: '),write(var_A),numDelivered(var_Num),var_N1 is var_Num+1,totOrder(var_Tot),var_N2 is var_Tot+1,retractall(numDelivered(var__)),assert(numDelivered(var_N1)),retractall(totOrder(var__)),assert(totOrder(var_N2)),nl,nl.

eve(orderNotFound(var_N)):-retractall(order(var_N,var__,var__,var__,var__)),assert(order(var_N,0,0,0,3)),retractall(stop(var__)),assert(stop(0)),write('IMPOSSIBILE CONSEGNARE IL PRODOTTO, AGENTI NON CONFORMI PER RICHIESTA:'),write(var_N),numNotDelivered(var_Num),var_N1 is var_Num+1,totOrder(var_Tot),var_N2 is var_Tot+1,retractall(numNotDelivered(var__)),assert(numNotDelivered(var_N1)),retractall(totOrder(var__)),assert(totOrder(var_N2)),nl,nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370721,var_Ontology,_370725),_370715),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_370759)),a(message(var_Ag,propose(var_A,[_370759],var_AgI))),retractall(ext_agent(var_Ag,_370797,var_Ontology,_370801)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370595,var_Ontology,_370599),_370589),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_370665,var_Ontology,_370669)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370483,var_Ontology,_370487),_370477),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_370539,var_Ontology,_370543)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_370047),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_370081),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_369863).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_369711),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_369659),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_369535,_369537)),clause(agente(_369557,_369559,_369561,var_S),_369553),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_369329,_369331,_369333,var_S),_369325),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_369429,[]),repeat,read(_369429,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_369429).
